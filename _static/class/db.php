...
class db {
	...
	public function __construct($dbms = DBMS,$name = DB['NAME'],$server = DB['SERVER'],$user = DB['USER'],$password = DB['PASSWORD'],$charset = 'utf8mb4') {
		$this->conn = $this->connect($dbms,$name,$server,$user,$password,$charset);
	}

	public function connect($dbms = DBMS,$name = DB['NAME'],$server = DB['SERVER'],$user = DB['USER'],$password = DB['PASSWORD'],$charset = 'utf8mb4') {
		$conn = false;
		switch(strtolower($dbms)) {
			...
			case 'mongo':
			case 'mongodb':
				$conn = new MongoDB\Driver\Manager('mongodb://'.$server.':27017');
				/*
	...
	public function query($query) {
		...
		if ($this->query = $this->conn->prepare($query)) {
			$this->debug_query = str_replace('?','"?"',$query);
					...
					foreach ($args as $k => &$arg) {
						if (is_array($args[$k])) {
							foreach ($args[$k] as $j => &$a) {
								...
								$this->debug_query = substr_replace($this->debug_query,$args_ref[sizeof($args_ref)-1],strpos($this->debug_query,'?'),1);
							}
						} else {
							echo strpos('?',$this->debug_query);
			...
			$this->query->execute();
			if ($this->query->errno) {
				$this->error('failed to execute: ' . $this->query->error .((DEBUG)?chr(10).chr(13).$this->debug_query:''));
			}
	...
	public function get($table) {
		switch($this->dbms) {
			...
			case 'mssql':
				...
				if(is_array($args_ref) && sizeof($args_ref) > 0) $query = array_merge(array($query),$args_ref);
	...
	public function count($table,$where = '',$vals = null,$fields = null) {
		...
		if(call_user_func_array(array($this,'query'),$query)) {
			...
			//return $this->rows();
			$result = $this->fetch();
			return (is_array($result) && sizeof($result) > 0)?$result[0]['CNT']:0;
		}
		else {
			return -1;
		}
	...
	public function table_exist($table) {
		...
		');
		if(call_user_func_array(array($this,'query'),$query)) {
			return intval($this->fetch()[0]['flag']);
		}
		else {
			return -1;
		}
