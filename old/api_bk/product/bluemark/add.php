<?php
/*
 +=============================================================================
 | 
 | 마이페이지 블루마크 - 블루마크 인증
 | -------
 |
 | 최초 작성	: 윤재은
 | 최초 작성일	: 2023.01.09
 | 최종 수정일	: 
 | 버전		: 1.0
 | 설명		: 
 | 
 +=============================================================================
*/

mysqli_report(MYSQLI_REPORT_ERROR | MYSQLI_REPORT_STRICT);

$country = null;
if (isset($_SESSION['COUNTRY'])) {
	$country = $_SESSION['COUNTRY'];
}

$member_idx = 0;
if (isset($_SESSION['MEMBER_IDX'])) {
	$member_idx = $_SESSION['MEMBER_IDX'];
}

$ip = '0.0.0.0';
if (isset($_SERVER['REMOTE_ADDR'])) {
	$ip = $_SERVER['REMOTE_ADDR'];
}

$mall_type = null;
if (isset($_POST['mall_type'])) {
	$mall_type = $_POST['mall_type'];
}

$serial_code = null;
if (isset($_POST['serial_code'])) {
	$serial_code = $_POST['serial_code'];
}

$purchase_mall = null;
if (isset($_POST['purchase_mall'])) {
	$purchase_mall = $_POST['purchase_mall'];
}

$purchase_date = null;
if (isset($_POST['purchase_date'])) {
	$purchase_date = $_POST['purchase_date'];
}

if ($member_idx == 0) {
	$json_result['code'] = 401;
	$json_result['msg'] = "로그인 후 다시 시도해 주세요.";
	
	return $json_result;
}

if(!($country != null && $member_idx > 0)){
	$json_result['code'] = 302;
	$json_result['msg'] = '로그인 정보가 없습니다.';
	return $json_result;
}

if($purchase_date == null && intval($mall_type) != 1) {
	$json_result['code'] = 302;
	$json_result['msg'] = '구매일 확인에 실패했습니다.';
	return $json_result;
}

if($serial_code != null && $purchase_mall != null) {
	$db->begin_transaction();
	try {
		$bluemark_cnt = $db->count("BLUEMARK_INFO BI", "BI.SERIAL_CODE ='".$serial_code."' AND BI.MEMBER_IDX = 0 AND BI.STATUS = FALSE AND BI.DEL_FLG = FALSE");
		
		if($bluemark_cnt > 0) {
			$update_bluemark_sql = "
				UPDATE
					BLUEMARK_INFO,
					(
						 SELECT
								MB.IDX				AS TMP_MEMBER_IDX,
								MB.MEMBER_ID		AS TMP_MEMBER_ID,
								MB.MEMBER_NAME		AS TMP_MEMBER_NAME,
								MB.TEL_MOBILE		AS TMP_TEL_MOBILE,
								MB.MEMBER_ID		AS TMP_EMAIL,
								TRUE				AS TMP_STATUS,
								NOW()				AS TMP_UPDATE_DATE,
								MB.MEMBER_ID		AS TMP_UPDATER
						 FROM
								MEMBER_".$country." MB
						 WHERE
								MB.IDX = '".$member_idx."'
					) AS TMP
				SET
					MEMBER_IDX		= TMP.TMP_MEMBER_IDX,
					MEMBER_ID		= TMP.TMP_MEMBER_ID,
					MEMBER_NAME		= TMP.TMP_MEMBER_NAME,
					TEL_MOBILE		= TMP.TMP_TEL_MOBILE,
					EMAIL			= TMP.TMP_EMAIL,
					STATUS			= TMP.TMP_STATUS,
					UPDATE_DATE		= NOW(),
					UPDATER			= TMP_UPDATER
				WHERE
					SERIAL_CODE = '".$serial_code."'
			";
			
			$db->query($update_bluemark_sql);

			$db_result = $db->affectedRows();

			if ($db_result > 0) {
				$insert_bluemark_log_sql = "
					INSERT
						BLUEMARK_LOG
					(
						COUNTRY,
						BLUEMARK_IDX,
						MEMBER_IDX,
						MEMBER_ID,
						MEMBER_NAME,
						IP,
						STATUS,
						PURCHASE_MALL,
						PURCHASE_DATE,
						REG_DATE
					)
					SELECT
						'".$country."'  		AS COUNTRY,
						IDX						AS BLUEMARK_IDX,
						MEMBER_IDX				AS MEMBER_IDX,
						MEMBER_ID				AS MEMBER_ID,
						MEMBER_NAME				AS MEMBER_NAME,
						'".$ip."'				AS IP,
						'신규인증'				AS STATUS,
						'".$purchase_mall."'	AS PURCHASE_MALL,
						'".$purchase_date."'	AS PURCHASE_DATE,
						NOW()					AS REG_DATE
					FROM
						BLUEMARK_INFO BI
					WHERE 
						BI.SERIAL_CODE = '".$serial_code."'
				";
				$db->query($insert_bluemark_log_sql);
			}
		} else {
			$db->rollback();
			$json_result['code'] = 301;
			$json_result['msg'] = '인증하려는 블루마크의 정보가 유효하지 않습니다.';
			return $json_result;
		}
		$db->commit();
	} catch(mysqli_sql_exception $exception){
		$db->rollback();

		$json_result['code'] = 302;
		$json_result['msg'] = '블루마크 신규 인증에 실패했습니다.';
		
		return $json_result;
	}
} 
else {
	$json_result['code'] = 302;
	$json_result['msg'] = '블루마크 필수항목값이 비어있습니다.';
	return $json_result;
}

?>