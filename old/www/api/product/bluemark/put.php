...
if (isset($country) && $member_idx > 0 && isset($handover_id) && isset($bluemark_idx)) {
	...
	try {
		...
		$cnt_handover = $db->count("MEMBER_".$country." MB", "MB.MEMBER_ID = '".$handover_id."'");
		
		if ($cnt_bluemark > 0 && $cnt_handover > 0) {
			...
			$db->query($update_bluemark_info_sql,array($bluemark_idx));
			...
			if ($db_result > 0) {
				...
				$db->query($insert_bluemark_log_sql,array($bluemark_idx));
