...
function getDeliveryPrice($db,$country,$to_country_code) {
	...
	$db->query($select_dhl_zones_sql,array($country,$to_country_code));
...
function getTotalBasketPrice($db,$country,$member_idx,$basket_idx,$to_country_code) {
	...
	$db->query($select_basket_info_sql,array($country,$member_idx));
...
function getTotalMileagePrice($db,$country,$member_idx) {
	...
	$db->query($select_total_mileage_sql,array($country,$member_idx));
...
/* 국가별 적립금 사용 가능금액 체크 */
function checkBasketPrice($country,$total_basket_price) {
	$check_result = false;
	
	if ($country == "KR") {
		/* 적립금 사용 가능금액 : 80000KRW 이상 */
		if ($total_basket_price >= 80000) {
			$check_result = true;
		}
	} else {
		/* 적립금 사용 가능금액 : 300USD 이상 */
		if ($total_basket_price >= 300) {
			$check_result = true;
		}
	}
