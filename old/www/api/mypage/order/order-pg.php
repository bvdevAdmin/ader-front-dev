...
function getOrderPriceInfo($db,$order_status,$order_code) {
	...
	if ($order_status != "PRD") {
		if ($order_status == "OCC") {
			$column_price_delivery = "
				(
					SUM(OT.PRICE_DELIVERY) - SUM(OT.DELIVERY_RETURN)
				) AS PRICE_DELIVERY,
			";
		} else {
			$column_price_delivery = "
		...
		$db->query($select_order_price_sql,array($order_code));
		
		foreach($db->fetch() as $data) {
			$order_price = array(
				'price_product'			=>number_format($data['PRICE_PRODUCT']),
				'price_cancel'			=>number_format($data['PRICE_CANCEL']),
				'price_discount'		=>number_format($data['PRICE_DISCOUNT']),
				'price_mileage_point'	=>number_format($data['PRICE_MILEAGE_POINT']),
				'price_charge_point'	=>number_format($data['PRICE_CHARGE_POINT']),
				'price_delivery'		=>number_format($data['PRICE_DELIVERY']),
		...
		$select_order_price_sql = "
			SELECT
				...
				J_OCC.PRICE_PRODUCT			AS C_PRICE_PRODUCT,
				J_OCC.PRICE_MILEAGE_POINT	AS C_PRICE_MILEAGE_POINT,
				J_OCC.PRICE_CHARGE_POINT	AS C_PRICE_CHARGE_POINT,
				J_OCC.PRICE_DISCOUNT		AS C_PRICE_DISCOUNT,
				0							AS C_PRICE_DELIVERY,
				J_OCC.PRICE_CANCEL			AS C_PRICE_CANCEL,
				J_OCC.PRICE_REFUND			AS C_PRICE_REFUND,
				
				J_ORF.PRICE_PRODUCT			AS R_PRICE_PRODUCT,
				J_ORF.PRICE_MILEAGE_POINT	AS R_PRICE_MILEAGE_POINT,
				J_ORF.PRICE_CHARGE_POINT	AS R_PRICE_CHARGE_POINT,
				J_ORF.PRICE_DISCOUNT		AS R_PRICE_DISCOUNT,
				J_ORF.PRICE_DELIVERY		AS R_PRICE_DELIVERY,
				J_ORF.PRICE_CANCEL			AS R_PRICE_CANCEL,
				J_ORF.PRICE_REFUND			AS R_PRICE_REFUND
			FROM
				ORDER_INFO OI
				
				LEFT JOIN (
					SELECT
						S_OCC.ORDER_CODE				AS ORDER_CODE,
						SUM(PRICE_PRODUCT)				AS PRICE_PRODUCT,
						SUM(PRICE_MILEAGE_POINT)		AS PRICE_MILEAGE_POINT,
						SUM(PRICE_CHARGE_POINT)			AS PRICE_CHARGE_POINT,
						SUM(PRICE_DISCOUNT)				AS PRICE_DISCOUNT,
						SUM(S_OCC.PRICE_CANCEL)			AS PRICE_CANCEL,
						SUM(S_OCC.PRICE_REFUND)			AS PRICE_REFUND
					FROM
						ORDER_CANCEL S_OCC
					GROUP BY
						S_OCC.ORDER_CODE
				) AS J_OCC ON
				OI.ORDER_CODE = J_OCC.ORDER_CODE
				
				LEFT JOIN (
					SELECT
						S_ORF.ORDER_CODE				AS ORDER_CODE,
						SUM(S_ORF.PRICE_PRODUCT)		AS PRICE_PRODUCT,
						SUM(S_ORF.PRICE_MILEAGE_POINT)	AS PRICE_MILEAGE_POINT,
						SUM(S_ORF.PRICE_CHARGE_POINT)	AS PRICE_CHARGE_POINT,
						SUM(S_ORF.PRICE_DELIVERY)		AS PRICE_DELIVERY,
						SUM(S_ORF.PRICE_DISCOUNT)		AS PRICE_DISCOUNT,
						SUM(S_ORF.PRICE_CANCEL)			AS PRICE_CANCEL,
						SUM(S_ORF.PRICE_REFUND)			AS PRICE_REFUND
					FROM
						ORDER_REFUND S_ORF
					GROUP BY
						S_ORF.ORDER_CODE
				) AS J_ORF ON
				OI.ORDER_CODE = J_ORF.ORDER_CODE
			WHERE
				OI.ORDER_CODE = ?
		";
		...
		foreach($db->fetch() as $data) {
			$order_price = array(
				'price_product'			=>number_format($data['PRICE_PRODUCT'] - $data['C_PRICE_PRODUCT'] - $data['R_PRICE_PRODUCT']),
				'price_cancel'			=>0,
				'price_discount'		=>number_format($data['PRICE_DISCOUNT'] - $data['C_PRICE_DISCOUNT'] - $data['R_PRICE_DISCOUNT']),
				'price_mileage_point'	=>number_format($data['PRICE_MILEAGE_POINT'] - $data['C_PRICE_MILEAGE_POINT'] - $data['R_PRICE_MILEAGE_POINT']),
				'price_charge_point'	=>number_format($data['PRICE_CHARGE_POINT'] - $data['C_PRICE_CHARGE_POINT'] - $data['R_PRICE_CHARGE_POINT']),
				'price_delivery'		=>number_format($data['PRICE_DELIVERY'] - $data['C_PRICE_DELIVERY'] - $data['R_PRICE_DELIVERY']),
				'price_total'			=>number_format($data['PRICE_TOTAL'] - $data['C_PRICE_REFUND'] - $data['R_PRICE_REFUND'])
			);
		}
	}
