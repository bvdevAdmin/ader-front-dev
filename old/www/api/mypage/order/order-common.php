...
function getOrderProductByStatus($db,$order_status,$order_code,$select_type) {
	...
	if ($order_status != "OCC" || $order_status != "ORF") {
		$column_delivery = "
	...
	foreach($db->fetch() as $product_data) {
		$param_url = array(
			'company_code'		=>$product_data['COMPANY_CODE'],
			'delivery_num'		=>$product_data['DELIVERY_NUM']
		);
		
		$url_delivery = setURL_delivery($param_url);
...
function setURL_delivery($param) {
	...
	if ($param['company_code'] != null && ($param['delivery_num'] != null && $param['delivery_num'] != "-")) {
