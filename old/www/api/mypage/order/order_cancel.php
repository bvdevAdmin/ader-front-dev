...
if ($member_idx > 0 && isset($order_code)) {
	try {
		...
		if (!empty($order_idx)) {
			...
			/* ========== NAVER CLOUD PLATFORM::주문 취소처리 메일 발송 ========== */
			/* PARAM::MAIL */
			$param_mail = array(
				'country'		=>$country,
				'mail_type'		=>"M",
				'mail_code'		=>"MAIL_CODE_0006",
...
function getParamCancelPrice($db,$order_pg_info,$param_order_product,$order_code,$price_mileage) {
	...
	for ($i=0; $i<count($param_order_product); $i++) {
		...
		$db->query($select_order_product_sql,array($data['param_idx']));
...
function checkTotalCancel($db,$order_code,$price_product,$param_price_product) {
	...
	$db->query($select_total_cancel_price_sql,array($order_code));
...
function addProductCancel($db,$param) {
	...
	for ($i=0; $i<count($product_info); $i++) {
		...
		$db->query($insert_order_product_cancel_sql,array($data['order_product_code']));
	}
}
...
function addOrderCancel($db,$param) {
	...
	$db->query($insert_order_cancel_sql,array($param['order_code']));
...
function putOrderProduct($db,$param_order_product,$member_id) {
	for ($i=0; $i<count($param_order_product); $i++) {
		...
		$db->query($update_order_product_sql,array($data['param_idx']));
	}
}
...
function addCancelMileageInfo($db,$order_update_code,$price_mileage,$member_id) {
	...
	$db->query($insert_mileage_info_sql,array($order_update_code));
}
