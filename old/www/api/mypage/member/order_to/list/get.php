...
if (isset($country) && $member_idx > 0) {
    $select_order_to_sql = "
        SELECT
            ...
            IFNULL(CI.COUNTRY_NAME,'') AS TO_COUNTRY_NAME,
            IFNULL(PI.PROVINCE_NAME,'') AS TO_PROVINCE_NAME,
            OT.TO_CITY AS TO_CITY,
            OT.TO_ADDRESS AS TO_ADDRESS,
            OT.DEFAULT_FLG AS DEFAULT_FLG
        FROM
            ORDER_TO OT
        LEFT JOIN COUNTRY_INFO CI ON OT.TO_COUNTRY_CODE = CI.COUNTRY_CODE
        LEFT JOIN PROVINCE_INFO PI ON OT.TO_PROVINCE_IDX = PI.IDX
        WHERE
            OT.COUNTRY = ? AND OT.MEMBER_IDX = ?
        ORDER BY
            OT.IDX DESC
    ";

    $db->query($select_order_to_sql, [$country, $member_idx]);
