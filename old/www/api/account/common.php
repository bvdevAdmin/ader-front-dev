	...
	if ($check_pw) {
		...
		if ($check_status) {
			...
			if ($check_ip) {
				...
				setMemberLoginHistory($db, $member);
			} else {
...
function setMemberLoginHistory($db, $member){
	$param = [$member['country']];
	$param[] = $member['member_idx'];
	$param[] = $member['member_ip'];
	$param[] = $member['member_id'];
	$param[] = $member['member_name'];
	$param[] = $member['login_cnt'] + 1;
	...
	$db->query($insert_login_history,$param);
    ...
    if ($member['member_birth'] != null) {
        ...
        if ($now_m == $birth_m) {
            ...
            $cnt_issue = $db->count(
                "
                    VOUCHER_ISSUE VI
                    LEFT JOIN VOUCHER_MST VM ON VI.VOUCHER_IDX = VM.IDX
                ",
                "
                    ...
                    VI.CREATE_YEAR = ?
                ",
                [$country, $member['member_idx'], $now_y]
            );
...
function issueBirthVoucher($db, $param) {
    ...
    $db->query($select_voucher_mst_sql, [$param['country']]);
        ...
        if (!empty($voucher_idx)) {
            ...
            if ($issue_cnt == 0) {
                ...
                if (!empty($voucher_issue_idx)) {
                    ...
                    $db->query($update_voucher_mst_sql, [$voucher_idx, $voucher_idx]);
                }
            }
        }
    }
