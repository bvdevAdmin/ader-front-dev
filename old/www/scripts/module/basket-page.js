...
function BasketPage(el) {
	...
	function getBasketProductList() {
		$.ajax({
			...
			success: function (d) {
				...
				/* 쇼핑백 화면 배송비 설정 */
				let deli_price = document.querySelector('.deli__price');
				deli_price.dataset.price_delivery = delivery_price;
				deli_price.innerText = txt_delivery_price;
		...
		function productNull() {
			...
			let list__box = document.querySelector(".basket__wrap .list__box");
			list__box.style.gridColumn = "7/11";
			list__box.style.width = "100%";
	...
	function createColorLine(product_idx,data,status) {
		...
		if (status == "product") {
			if (colorMulti.length > 1) {
				colorLineHtml = `
					<div class="color-line" data-product_idx="${product_idx}" style="--background:linear-gradient(90deg, ${colorMulti[0]} 50%, ${colorMulti[1]} 50%);">
						<div class="color multi" data-soldout="${data.stock_status}" data-title="${data.color}"></div>
					</div>
				`;
			} else {
				colorLineHtml = `
					<div class="color-line" data-product_idx="${product_idx}" data-title="${data.color}" style="--background-color:${colorMulti[0]}" >
			...
			if (colorMulti.length > 1) {
				...
				colorLineHtml = `
					<div class="color-line" data-product_idx="${product_idx}" style="--background-color:${colorMulti[0]}" >
	...
	function writeProductListDomTree(st_info, so_info) {
		...
		/* 재고있음 상태의 쇼핑백 상품 리스트 */
		if (st_info != null && st_info.length > 0) {
			st_info.forEach(el => {
				...
				write_STIN_HTML += `
					<div class="product__box product_box_${el.basket_idx}" data-stock_status="${el.stock_status}"  data-basket_idx="${el.basket_idx}" data-basket_qty="${el.basket_qty}" data-product_idx="${el.product_idx}" data-option_idx="${el.option_idx}" data-product_qty="${el.product_qty}">
						...
						<div class="prd__content" data-sales_price="${el.sales_price}" >
							...
							<div class="prd__qty">
								<div>Qty</div>
								<div class="minus__btn ${qty_disable}" data-product_type="${el.product_type}">
									<img src="/images/svg/minus-basket.svg">
								</div>
								
								<input class="count__val" type="text" value="${el.basket_qty}" readonly>
								
								<div class="plus__btn ${qty_disable}" data-product_type="${el.product_type}">
		...
		/* 재고없음 상태의 쇼핑백 상품 리스트 */
		if (so_info != null && so_info.length > 0) {
			let docFrag = document.createDocumentFragment();
			...
			so_info.forEach(el => {
				...
				if (product_type == "B") {
					...
					product_size.forEach((size) => {
						...
						change_product_size_HTML += `
							<li class="option__size ${disable_btn}" data-product_idx="${size.product_idx}" data-option_idx="${size.option_idx}" data-stock_status="${size.stock_status}">
								${size.option_name}
							</li>
						`;
					});
						...
						product_size.forEach(option => {
							...
							change_product_color_HTML += `
								<div class="product_option_wrap">
									...
									<div class="color__box">
										${setOptionInfo.map(row => {return createColorLine(el.product_idx,row, "option");}).join("")}
									</div>
								</div>
							`;
						});
				...
				if (el.discount > 0) {
					soldout_price_HTML = `
						<div class="price" data-soldout="${el.stock_status}" data-sales_price="${el.sales_price}" data-discount="${el.discount}" data-dis="true"><span>${el.price_txt}</span></div>
					`;
				} else {
					soldout_price_HTML = `
						...
						<div class="prd__content">
							...
							<div class="option__box">
								...
								<div class="reorder__btn">
									...
									<u class="${reorder_class}">${reorder_text}</u>
								</div>
							</div>
	...
	function payBoxSumPrice(price_product) {
		...
		if (getLanguage() == "KR" && free_delivery <= price_product) {
	...
	function addReorderInfo(basket_idx, product_idx, option_idx) {
		$.ajax({
			...
			success: function (d) {
				let code = d.code;
				if (code == 200) {
					setReorderFlg(product_idx,option_idx);
				} else {
					makeMsgNoti(getLanguage(), 'MSG_F_ERR_0028', null);
				}
			}
	...
	function setReorderFlg(param_product_idx,param_option_idx) {
		let div_STSO_product_box = document.querySelectorAll('.sold__list__box .product__box');
		div_STSO_product_box.forEach(box => {
			let product_idx = box.dataset.product_idx;
			let option_idx = box.dataset.option_idx;
			
			if (product_idx == param_product_idx && option_idx == param_option_idx) {
				let reorder_btn = box.querySelector(".reorder__btn u");
				reorder_btn.innerHTML = "재입고 알림 신청완료";
				reorder_btn.dataset.i18n = "w_basket_msg_04";
				reorder_btn.textContent = i18next.t("w_basket_msg_04");
				reorder_btn.classList.add('disableBtn');
			}
		});
