...
const imgSwiper = (move) => {
	...
	if (typeof (product_lisw_swipe) == 'object') [...product_lisw_swipe].map(el => el.destroy());

	return product_lisw_swipe = new Swiper('.product-img', {
		// autoHeight: true,
		grabCursor: true,
		slidesPerView: 1,
		observer: true,
...
function writeWishBtnHtml(wish_flg, product_idx) {
	...
	wish_btn_html = `<div class="wish__btn" product_idx="${product_idx}">${whish_img}</div>`;

	return wish_btn_html;
}

function writeProductImgSlide(img_param, slide_type, product_idx, data) {
	...
	data.forEach(img => {
		product_img_slide_html += `
			<div class="swiper-slide product_img_swiper" data-imgtype="${img_type}" ${slide_hidden}>
				<img class="prd-img" cnt="${product_idx}" src="${cdn_img}${img.img_location}" alt="">
			</div>
		`;
	});
    ...
    data.forEach(el => {
        if (el.grid_type == "PRD") {
            ...
            if (p_img != null && p_img.length > 0) {
                product_p_slide_html = writeProductImgSlide(img_param, "P", product_idx, p_img);
            }

            if (o_img != null && o_img.length > 0) {
                product_o_slide_html = writeProductImgSlide(img_param, "O", product_idx, o_img);
            } else {
            ...
            product_list_html += `
                <div class="product prd op_0">
                    ${wish_btn_html}
                    <a href="http://116.124.128.246:80/product/detail?product_idx=${product_idx}">
...
function swiperStateCheck() {
	...
	if (isWideScreen) {
		if (webpre === "2") {
			imgSwiper(true);
		} else if (webpre === "4" && product_lisw_swipe !== undefined) {
			product_lisw_swipe.forEach(el => el.disable());
		}
	} else {
		if (mobilepre === "1") {
			imgSwiper(true);
		} else if (mobilepre === "2" || mobilepre === "3") {
			if (product_lisw_swipe !== undefined) {
				product_lisw_swipe.forEach(el => el.disable());
			}
		}
	}
...
//상품 더 불러오기
function getProductListByScroll(filter_param) {
	if (more_flg == false) {
		let product_list_wrap = document.querySelector('.product__list__wrap');
		...
		let show_more_btn_html = `
			<div class="show_more_btn">
				<span class="add-btn" data-i18n="pl_view_more">더보기 +</span>
			</div>
		`;
