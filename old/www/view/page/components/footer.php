<!-- 모바일 -->
<ul class="footer-mobile">
	<li class="about drop__menu">
		<div class="drop__down__wrap">
			<span class="drop-title">About ADERERROR</span>
			<div class="drop__btn plus-btn">
				<span class="plus-line"></span>
				<span class="plus-line"></span>
			</div>
		</div>
		<div class="drop__down__content">
			<p class="about">ADERERROR (아더에러)는 2014년 설립되었으며 패션을 기반으로 한 문화 커뮤니케이션 브랜드입니다. ADERERROR는 ‘but near missed things’ 이라는 브랜드 슬로건, 철학을 바탕으로 사람들이 일상에서 쉽게 놓치고 있는 것들을 익숙하지만 낯설고, 새롭게 느낄 수 있도록 표현하는 활동에 집중하고 있으며, 사진, 영상, 공간, 디자인, 예술, 가구 등 문화 콘텐츠를 우리의 방식으로 재편집하여 새로운 문화를 제안합니다. ADER는 모든 영역 간의 커뮤니케이션 디자인하는 것을 브랜드 핵심 가치로서 추구합니다.</p>
		</div>
	</li>
	
	<li class="footer__content drop__menu">
		<div class="drop__down__wrap">
			<span class="drop-title" style="font-size: 1.2rem;" data-i18n="lm_regulations">법적 고지사항</span>
			<div class="drop__btn plus-btn">
				<span class="plus-line"></span>
				<span class="plus-line"></span>
			</div>
		</div>
		<div class="drop__down__content">
			<ul>
				<li class="disclaimer" onclick="location.href='/notice/privacy?notice_type=online_store'" data-i18n="lm_online_store_guide">온라인 스토어 이용가이드</li>
				<li class="disclaimer" onclick="location.href='/notice/privacy?notice_type=terms_of_use'" data-i18n="lm_terms_and_conditions">이용 약관</li>
				<li class="disclaimer" onclick="location.href='/notice/privacy?notice_type=privacy_policy'" data-i18n="lm_privacy_policy_02">개인정보 처리방침</li>
			</ul>
		</div>
	</li>
	
	<li class="footer__content drop__menu">
		<div class="drop__down__wrap">
			<span class="drop-title"style="font-size: 1.2rem;" data-i18n="lm_social_media">소셜 미디어</span>
			<div class="drop__btn plus-btn">
				<span class="plus-line"></span>
				<span class="plus-line"></span>
			</div>
		</div>
		<div class="drop__down__content">
			<div class="social">
				<div class="svg-wrap">
					<img class="wechat_qr_btn so-svg" src="/images/svg/wechat.svg" alt="">
					<a href="https://www.facebook.com/adererror" target="_blank"><img class="so-svg" src="/images/svg/facebook.svg" alt=""></a>
					<a href="http://instagram.com/ader_error#" target="_blank"><img class="so-svg" src="/images/svg/instagram.svg" alt=""></a>
					<a href="https://goo.gl/s1b2mN" target="_blank"><img class="so-svg" src="/images/svg/youtube.svg" alt=""></a>
				</div>
				<div class="svg-wrap">
					<a href="http://pf.kakao.com/_mQzRxl" target="_blank"><img class="so-svg" src="/images/svg/kakao-ch.svg" alt=""></a>
					<a href="https://pinterest.com/adererror/" target="_blank"><img class="so-svg" src="/images/svg/pinterest.svg" alt=""></a>
					<a href="https://goo.gl/MUkFap" target="_blank"><img class="so-svg" src="/images/svg/vimeo.svg" alt=""></a>
					<a href="http://weibo.com/aderofficial" target="_blank"><img class="so-svg" src="/images/svg/weibo.svg" alt=""></a>
				</div>
			</div>
		</div>
	</li>
	
	<li class="footer__content drop__menu">
		<div class="drop__down__wrap">
			<span class="drop-title"style="font-size: 1.2rem;" data-i18n="lm_customer_care_center">고객센터</span>
			<div class="drop__btn plus-btn">
				<span class="plus-line"></span>
				<span class="plus-line"></span>
			</div>
		</div>
		<div class="drop__down__content">
			<p class="service">ADER 3F 53, Yeonmujang-gil,</p>
			<!--<p class="service">Seongdong-gu, Seoul, Korea</p>-->
			<p class="service">TEL. 02-792-2232</p>
			<p class="service">Office hour Mon - Fri AM 10:00 - PM 5:00</p>
		</div>
	</li>
	
	<li class="footer__content drop__menu inf_content_m">
		<div class="drop__down__wrap">
			<span class="drop-title"style="font-size: 1.2rem;" data-i18n="lm_company">회사정보</span>
			<div class="drop__btn plus-btn">
				<span class="plus-line"></span>
				<span class="plus-line"></span>
			</div>
		</div>
		<div class="drop__down__content">
			<p class="info">Company | ADER</p>
			<p class="info">Business Name | FIVE SPACE CO.,LTD</p>
			<p class="info">Business License | 760-87-01757</p>
			<p class="info">Mail-order License No. | 제 2021-서울성동-01588호</p>
			<p class="info">CEO | HANN </p>
			<p class="info">OFFICE | ADER 3F 53, Yeonmujang-gil, Seongdong-gu, Seoul, Korea</p>
		</div>
	</li>
	<li class="last">
		<div class="drop__down__wrap">
			<span class="drop-title">© ADERERROR 2022</span>
		</div>
	</li>
</ul>
<div class="wechat_qr hidden"><img src="https://adererror.com/images/wechat-qrcode.png"></div>

<!-- 데스크탑 -->
<div class="footer-web">
	<ul class="f__desk">
		<li class="about" style="grid-column: 1/6;">
			
		</li>
		<li class="footer__content" style="grid-column: 8/10;">
			<div class="f__title">
				<span data-i18n="lm_regulations">법적 고지사항</span>
			</div>
			<div class="f_content">
				<ul>
					<li class="disclaimer" style="cursor:pointer" onclick="location.href='/notice/privacy?notice_type=online_store'" data-i18n="lm_online_store_guide">온라인 스토어 이용가이드</li>
					<li class="disclaimer" style="cursor:pointer" onclick="location.href='/notice/privacy?notice_type=terms_of_use'" data-i18n="lm_terms_and_conditions">이용 약관</li>
					<li class="disclaimer" style="cursor:pointer" onclick="location.href='/notice/privacy?notice_type=privacy_policy'" data-i18n="lm_privacy_policy_02">개인정보 처리방침</li>
				</ul>
			</div>
		</li>
		
		<li class="footer__content" style="grid-column: 10/11;">
			<div class="f__title">
				<span data-i18n="lm_social_media">소셜 미디어</span>
			</div>
			<div class="f_content">
				<div class="social">
					<div class="svg-wrap">
						<img class="wechat_qr_btn so-svg" src="/images/svg/wechat.svg" alt="">
						<a href="https://www.facebook.com/adererror" target="_blank"><img class="so-svg" src="/images/svg/facebook.svg" alt=""></a>
						<a href="http://instagram.com/ader_error#" target="_blank"><img class="so-svg" src="/images/svg/instagram.svg" alt=""></a>
						<a href="https://goo.gl/s1b2mN" target="_blank"><img class="so-svg" src="/images/svg/youtube.svg" alt=""></a>
					</div>
					<div class="svg-wrap">
						<a href="http://pf.kakao.com/_mQzRxl" target="_blank"><img class="so-svg" src="/images/svg/kakao-ch.svg" alt=""></a>
						<a href="https://pinterest.com/adererror/" target="_blank"><img class="so-svg" src="/images/svg/pinterest.svg" alt=""></a>
						<a href="https://goo.gl/MUkFap" target="_blank"><img class="so-svg" src="/images/svg/vimeo.svg" alt=""></a>
						<a href="http://weibo.com/aderofficial" target="_blank"><img class="so-svg" src="/images/svg/weibo.svg" alt=""></a>
					</div>
				</div>
			</div>
		</li>
		
		<li class="footer__content" style="grid-column: 12/14;">
			<div class="f__title">
				<span data-i18n="lm_customer_care_center">고객센터</span>
			</div>
			<div class="f_content">
				<p class="service">ADER 3F 53, Yeonmujang-gil,</p>
				<p class="service">Seongdong-gu, Seoul, Korea</p>
				<p class="service">TEL. 02-792-2232</p>
				<p class="service">Office hour Mon - Fri AM 10:00 - PM 5:00</p>
			</div>
		</li>
		
		<li class="footer__content inf_content" style="grid-column: 14/17; padding-left: 50px;">
		
		</li>
	</ul>
	<ul class="f__desk__last">
		<div class="f__title">
			<span>© ADERERROR 2022</span>
		</div>
	</ul>
</div>

<script>
	window.addEventListener('DOMContentLoaded', function() {
		dropMenuToggleBtn();
		showWechatQr();
		changeLanguage();
	});
	
	let country = localStorage.getItem('lang') || getLanguage();

	$.ajax({
		type: "post",
		url: api_location + "policy/footer/get",
		data: {
			"country": getLanguage()
		},
		dataType: "json",
		error: function() {
			alert("회사정보 조회에 실패했습니다.");
		},
		success: function(d) {
			let data = d.data;
			if(data != null) {
				data.forEach(el => {
					if(el.device_type == "W") {
						if(el.policy_type == "ABT") {
							let about_wrap = document.querySelector(".footer-web .about");
							about_wrap.innerHTML = el.policy_txt;
						} else if (el.policy_type == "INF") {
							let infWrap = $(".footer-web .inf_content");
							infWrap.replaceWith(el.policy_txt);
						}
					}
				})
			} else {
				alert("회사정보가 존재하지 않습니다.");
			}
		}
	});

	let dropMenuToggleBtn = () => {
		const  $$dropMenuBtn = document.querySelectorAll(".footer-mobile .drop__menu");
		const  $footerMobile = document.querySelector(".footer-mobile");
		$$dropMenuBtn.forEach(el => {
			el.addEventListener("click", function(e){

				if(e.currentTarget.classList.contains("show")){
					// e.currentTarget.classList.remove("show");
					// e.currentTarget.children[1].classList.remove("show");
					// e.currentTarget.children[0].children[1].classList.remove("toggle");
					allDropMenu();
				} else {
					allDropMenu();
					e.currentTarget.classList.add("show");
					e.currentTarget.children[0].classList.add("show");
					e.currentTarget.children[1].classList.add("show");
					e.currentTarget.children[0].children[1].classList.add("toggle");
				}
			});
		});
		function allDropMenu() {
			$$dropMenuBtn.forEach(el => {
				el.classList.remove("show");
				el.children[1].classList.remove("show");
				el.children[0].children[1].classList.remove("toggle");
			})
		}
	}
	function showWechatQr() {
		let qrBtn = document.querySelectorAll(".wechat_qr_btn");
		let qrImg = document.querySelector(".wechat_qr");

		qrBtn.forEach(btn => {
			btn.addEventListener("click", function() {
				qrImg.classList.remove("hidden");
			});
		});

		qrImg.addEventListener("click", function() {
			qrImg.classList.add("hidden");
		});
	}
</script>