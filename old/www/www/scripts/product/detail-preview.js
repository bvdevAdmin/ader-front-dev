...
const getProductInfo = (product_idx) => {
	$.ajax({
		...
		error: function () {
			...
			makeMsgNoti(getLanguage(), "MSG_F_ERR_0108", null);
		},
		success: function (d) {
			if (d.code == 200) {
				let data = d.data;
...
function makeProductListFlag(data) {
	...
	let wish_web_html = "";
	let wish_mobile_html = "";

	let wish_obj = {
		location: 'foryou',
		wishStatus: data.whish_flg,
		productIdx: data.product_idx,
	...
	if (login_status == "true") {
		if (whish_flg == 'true') {
			...
			whish_function = `updateWishlist(this,${JSON.stringify(wish_obj)});`;
	...
	wish_web_html = `
		<div class="wish-btn" product_idx="${data.product_idx}">
			${whish_img}
		</div>
	`;
	...
	const margin_box = document.createElement("div");
	margin_box.classList.add("margin_box");
	...
	info_wrap.insertBefore(margin_box, product_info);
		...
		success: function (d) {
			...
			if(code == 200) {
				...
				if(data != null) {
					...
					makeSizeGuideHtml(data, web);
					makeSizeGuideHtml(data, mobile);
...
function makeSizeGuideHtml(data, size) {
	...
	if (data != null && data.length > 0) {
		...
		data.forEach(function(row) {
			...
			contentBody.appendChild(updateSizeGuide(row));
		});
	}
}

function updateSizeGuide(row) {
	...
	sizeguideNoti.innerHTML = notiMsg;
	
	let img = document.createElement("img");

	img.setAttribute("src", `${cdn_img}${row.img_file_name}`);
	sizeguideImg.append(img);
		...
		dcts.forEach(dct => {
			dctHtml += `
				<li class="dct-row">
					<span>${dct.title}</span>
					<span>${dct.desc}</span>
					<span class="dct-value">${dct.value}</span>
				</li>
			`;
		});
	...
	sizeguideWrap.appendChild(sizeguideImg);
...
function viewportImg() {
	...
	closebtn.innerHTML = `
		<img src="https://s3-cloud-bucket-ader.s3.ap-northeast-2.amazonaws.com/s3-cloud-bucket-ader-user/images/svg/img-close-btn.svg">
	`
