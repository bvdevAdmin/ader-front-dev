...
if ($cnt_member == 0) {
	$cnt_user = $db->count("MEMBER_".$country,"TEL_MOBILE = '".$mobile."' ");
	if ($cnt_user > 0) {
		$db->update(
			"MEMBER_".$country,
			array(
		...
		foreach ($db->fetch() as $data) {
			...
			$param_start_date = $ago_year.$data['AGO_DATE_PARAM'];
			$param_end_date = $later_year.$data['LATER_DATE_PARAM'];
			$now_param = strtotime(date("Y-m-d"));
			
			if (strtotime($param_start_date) < $now_param && strtotime($param_end_date) > $now_param) {
				$param_issue = array(
					'country'		=>$country,
					'member_idx'	=>$data['MEMBER_IDX'],
				...
				issueBirthVoucher($db,$param_issue);
			}
