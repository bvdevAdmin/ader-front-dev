...
function callSEND_SMS($db,$sms,$data) {
	...
	/* 1. SMS 발송 PARAM */
	$param_sms = getPARAM_sms($db,$sms['country'],$sms['sms_code']);
	if ($param_sms != null) {
		/* 2. SMS 발송 대상 회원 PARAM */
		$member_sms = getMEMBER_sms($db,$sms['country'],$data['order_idx']);
		if ($member_sms != null) {
			/* 3. SMS 데이터 PARAM 조회 */
			$data_sms = getDATA_sms($db,$sms['country'],$data['order_idx']);
			if ($data_sms != null) {
				/* 4. SMS 데이터 항목 치환처리 */
