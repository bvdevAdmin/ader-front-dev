...
if (!isset($country) || $member_idx == 0) {
	...
	foreach($db->fetch() as $data){
		...
		$board_reply_info = getBoardReplyInfo($db,$board_idx);
		$board_img_info = getBoardImgInfo($db,$board_idx);
		...
		if ($data_type == "ENC") {
			$board_contents = xssEncode($board_contents);
		} else {
			$board_contents = xssDecode($board_contents);
		}
...
function getBoardImgInfo($db,$board_idx) {
	...
	return $board_img_info;
}

	
function getBoardReplyInfo($db,$board_idx) {
	...
	if (!empty($board_idx)) {
		...
		foreach($db->fetch() as $reply_data){
			$board_reply_info[] = array(
				'contents'		=>xssDecode($reply_data['CONTENTS'])
			);
		}
	}
	
	return $board_reply_info;
}
