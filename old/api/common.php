...
function addMEMBER_log($db,$member) {
	$insert_member_update_log_sql = "
		...
		SELECT
			...
			'".$member['member_status']."'	AS PREV_MEMBER_STATUS,
			MI.MEMBER_STATUS				AS MEMBER_STATUS,
			MI.MEMBER_ID					AS MEMBER_ID,
			MI.MEMBER_NAME					AS MEMBER_NAME,
			
			'".$member['member_pw']."'		AS PREV_MEMBER_PW,
			MI.MEMBER_PW					AS MEMBER_PW,
			'".$member['tel_mobile']."'		AS PREV_TEL_MOBILE,
	...
	$db->query($insert_member_update_log_sql,array($member['member_idx']));
}
