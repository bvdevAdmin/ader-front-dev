...
if (isset($country) && $member_idx > 0 && isset($order_idx) && isset($order_code) && isset($housing_type)) {
	$reason_pg_flg = false;
	...
	foreach($db->fetch() as $reason_data) {
		...
		$depth1_pg_cnt = $db->count("REASON_DEPTH_1","IDX = ".$depth1_idx." AND REASON_TYPE = '".$reason_data['ORDER_STATUS']."' AND PG_FLG = FALSE");
		$depth2_pg_cnt = $db->count("REASON_DEPTH_2","IDX = ".$depth2_idx." AND DEPTH_1_IDX = ".$depth1_idx." AND REASON_TYPE = '".$reason_data['ORDER_STATUS']."' AND PG_FLG = FALSE");
	...
	if ($add_tmp_flg == "T") {
		...
		putHousingInfo($db,$order_code,$housing_company,$housing_num);
	}
...
function putHousingInfo($db,$order_code,$housing_company,$housing_num) {
	...
	if ($cnt_TE > 0) {
		...
		$update_product_exchange_housing_sql = "
			UPDATE
				TMP_ORDER_PRODUCT_EXCHANGE
			SET
				DELIVERY_IDX = (SELECT IDX FROM DELIVERY_COMPANY WHERE COMPANY_NAME = '".$housing_company."'),
				DELIVERY_NUM = '".$housing_num."',
				DELIVERY_DATE = DATE_ADD(NOW(),INTERVAL 7 DAY),
				DELIVERY_START_DATE = NOW()
			WHERE
				ORDER_CODE = '".$order_code."'
		";
		
		$db->query($update_product_exchange_housing_sql);
	}
	...
	if ($cnt_TR > 0) {
		...
		$update_product_refund_housing_sql = "
			UPDATE
		...
		$db->query($update_product_refund_housing_sql);
	}
}
...
function addTmpDelivery($db,$order_status,$order_code,$order_update_code,$delivery_code,$delivery_title,$price_delivery,$member_id) {
	...
	$db->insert(
		...
		array(
			...
			"PRODUCT_NAME"			=>'주문 추가 배송비',
			
			"OPTION_IDX"			=>0,
			"BARCODE"				=>$delivery_code,
			"OPTION_NAME"			=>$delivery_title,
