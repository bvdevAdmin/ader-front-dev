			...
			if ($check_stock == true) {
				...
				if ($pg_info != null) {
					...
					if (isset($order_idx)) {
						...
						/* 알림톡 발송설정 체크처리 */
						$kakao_setting = checkKAKAO_setting($db,"KAKAO_CODE_0001");
						...
						/* 쇼핑몰 코드 PARAM 수정 - $_SERVER['HTTP_COUNTRY'] => "KR" */
						$mail_setting = checkMAIL_setting($db,"KR","MAIL_CODE_0004");
...
function initOrder($db,$order_code,$message_code) {
	...
	/* 쇼핑몰 코드 PARAM 수정 - $_SERVER['HTTP_COUNTRY'] => "KR" */
	$msg = getMsgToMsgCode($db,"KR",$message_code,array());
...
function checkOrder_tmp($db,$order_code) {
	...
	/* 쇼핑몰 코드 PARAM 수정 - $_SERVER['HTTP_COUNTRY'] => "KR" */
	$cnt_info		= $db->count("TMP_ORDER_INFO","ORDER_CODE = ? AND COUNTRY = ? AND MEMBER_IDX = ?",array($order_code,"KR",$_SESSION['MEMBER_IDX']));
	...
	foreach($db->fetch() as $data) {
		$db->insert(
			...
			array(
				...
				'PG_REMAIN_PRICE'		=>$pg['pg_remain_price'],
...
function addMileage_INC($db,$order_code,$mileage_per) {
	...
	foreach($db->fetch() as $data) {
		...
		$sales_price = $data['SALES_PRICE_KR'] * $data['PRODUCT_QTY'];
