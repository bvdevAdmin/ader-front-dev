...
const session_order		= `order_page_${data.depth}_${data.header_idx}`
const session_filter	= `filter_page_${data.depth}_${data.header_idx}`
const session_img		= `img_page_${data.depth}_${data.header_idx}`;
const session_grid		= `grid_page_${data.depth}_${data.header_idx}`;

let order_page = sessionStorage.getItem(session_order);
if (order_page != null) {
	order_page = JSON.parse(order_page);
	data.param_sort = order_page;
}

let filter_page = sessionStorage.getItem(session_filter);
if (filter_page != null) {
	$('.tools > ul > li > button.filter .icon').addClass('on');
	filter_page = JSON.parse(filter_page);
	data.param_filter = filter_page;
}
else{
	$('.tools > ul > li > button.filter .icon').removeClass('on');
}

let img_page = sessionStorage.getItem(session_img);
if (img_page != null) {
	if (img_page == "P") {
...
let grid_page = sessionStorage.getItem(session_grid);
if (grid_page != null) {
	if (grid_page == "2") {
	...
	$.ajax({
		...
		success: function(d) {
			if(d.code == 200) {
				...
				product_count = d.data.product_count;
				...
				if($("#goods-category > .swiper-wrapper > .swiper-slide").length == 0) {
					if ($("#goods-nav-top > ul > li").length == 0) {
						let menu_top = d.data.menu_info.menu_top;
						if (menu_top != null && menu_top.length > 0) {
							$("#goods-nav > ul").html('');
							menu_top.forEach(row => {
								$("#goods-nav-top > ul").append(`
									<li>
										<a href="${config.base_url}${row.menu_link}" class="on">
					...
					if ($("#goods-nav-middle > ul > li").length == 0) {
						let menu_middle = d.data.menu_info.menu_middle;
						if (menu_middle != null && menu_middle.length > 0) {
							$("#goods-nav > ul").html('');
							menu_middle.forEach(row => {
								$("#goods-nav-middle > ul").append(`
									<li>
										<a href="${config.base_url}${row.menu_link}" class="on">
											<span class="title${row.selected ? ' now': ''}">${row.menu_title}</span>
										</a>
									</li>
								`);
							})
					...
					if ($("#goods-nav-bottom > ul > li").length == 0) {
						let menu_bottom = d.data.menu_info.menu_bottom;
						if (menu_bottom != null && menu_bottom.length > 0) {
							$("#goods-nav-sub > ul").html('');
							menu_bottom.forEach(row => {
								$("#goods-nav-bottom > ul").append(`
									<li>
										<a href="${config.base_url}${row.menu_link}" class="on">
											<span class="title">${row.menu_title}</span>
										</a>
					...
					if ($("#goods-category > .swiper-wrapper > li").length == 0) {
						let menu_slide = d.data.menu_info.menu_slide;
						if (menu_slide != null && menu_slide.length > 0) {
							$("#goods-category > .swiper-wrapper").html('');
							const menu = menu_slide.map(row => {
								return `
									<li class="swiper-slide">
										<a href="${config.base_url}${row.slide_link}" class="on">
											<span class="image" style="background-image:url('${config.cdn + row.img_location}');"></span>
											<span class="title">${row.slide_title}</span>
										</a>
									</li>
								`;
							});

							$("#goods-category > .swiper-wrapper").append(menu);
						}
					}
					
					// 20250408 윤재은 - let 선언 제거
					new Swiper("#goods-category",{
						slidesPerView : 'auto',
						loop: false,
				...
				if(!d.data.grid_info) d.data.grid_info = []
				if( is_list_clear ) $('#list').empty()
				d.data.grid_info.forEach(row => {
					let sizeClass = "h1 w1";
					if (row.grid_size === "2x2") sizeClass = "h2 w2";
					if (row.grid_size === "2x1") sizeClass = "h1 w2";

					// 상품인 경우
					if(row.grid_type === 'PRD'){
						// 이미지 슬라이드
						...
						$("#list").append(`
							<li class="${(row.stock_status == 'STSO')?'soldout':''} ${sizeClass}" style="${(row.background_color!='')?'background-color:' + row.background_color:''}">
						...
						// 		<a href="${config.base_url}/shop/${row.product_idx}">
						// 			<span 
						// 				class="image item-${row.product_idx}" 
						...
						const filePath = `${config.cdn}${row.contents_location}`;
						if (row.asset_type === 'VID') {
							mediaTag = `<video src="${filePath}" autoplay muted loop playsinline></video>`;
						} else {
							mediaTag = `<img src="${filePath}" alt="asset" />`;
						}
						
						last_idx = row.display_num;
						$("#list").append(`
							<li class="${sizeClass} asset" style="${(row.background_color!='')?'background-color:' + row.background_color:''}">
								${mediaTag}
							</li>
						`);
