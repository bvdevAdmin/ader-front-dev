...
let keyword = location.pathname.split("/")[3]; // 검색 키워드
...
const session_order	= `order_search_${keyword}`
const session_filter	= `filter_search_${keyword}`
const session_img		= `img_search_${keyword}`;
const session_grid	= `grid_search_${keyword}`;

let order_search = sessionStorage.getItem(session_order);
if (order_search != null) {
	order_search = JSON.parse(order_search);
	data.param_sort = order_search;
}

let filter_search = sessionStorage.getItem(session_filter);
if (filter_search != null) {
	$('.tools > ul > li > button.filter .icon').addClass('on');
	filter_search = JSON.parse(filter_search);
	data.param_filter = filter_search;
}
else{
	$('.tools > ul > li > button.filter .icon').removeClass('on');
}

let img_search = sessionStorage.getItem(session_img);
if (img_search != null) {
	if (img_search == "P") {
		$("ul#list").parent().removeClass("outfit");
		$('main.goods.list > header section.tools button.showing').removeClass('on');
	} else if (img_search == "O") {
		...
		success: function(d) {
			if(d.code == 200) {
				if ($("#goods-nav-top > ul > li").length == 0) {
					const decodedKeyword = decodeURIComponent(data.keyword);
					$("#goods-nav > ul").html('');
					...
					if (config.language != "KR") {
						t_home		= "Home";
						t_result	= "Search result";
					}

					$("#goods-nav-top > ul").append(`
						<li>
							<a href="/kr" class="on">
								<span class="title">${t_home}</span>
							</a>
						</li>
						<li>
							<a href="#" class="on">
								<span class="title">'${decodedKeyword}' ${t_result}</span>
							</a>
						</li>
					`);
				}
				...
				d.data.grid_info.forEach(row => {
					...
					last_idx++;
					data.last_idx = last_idx;
					console.log(' [ get_goods ] ',last_idx);
					$("#list").append(`
...
function getFilter() {
	$.ajax({
		...
		data: {
			'filter_type'	: 'search',
			'filter_param'	: decodeURIComponent(data.keyword)
		},
		async: false,
		beforeSend: function(xhr) {
			xhr.setRequestHeader("country",config.language);
...
function getTotal() {
	$.ajax({
		...
		data: {
			'total_type'	: 'search',
			'total_param'	: decodeURIComponent(data.keyword)
		},
		async: false,
		beforeSend: function(xhr) {
			xhr.setRequestHeader("country",config.language);
