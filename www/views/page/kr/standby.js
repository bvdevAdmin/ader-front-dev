...
let uuid = location.search.split("=")[1] //스탠바이 uuid
...
const session_order		= `order_standby_${data.depth}_${data.header_idx}`
const session_filter	= `filter_standby_${data.depth}_${data.header_idx}`
const session_img		= `img_standby_${data.depth}_${data.header_idx}`;
const session_grid		= `grid_standby_${data.depth}_${data.header_idx}`;

let order_standby = sessionStorage.getItem(session_order);
if (order_standby != null) {
	order_standby = JSON.parse(order_standby);
	data.param_sort = order_standby;
}

let filter_standby = sessionStorage.getItem(session_filter);
if (filter_standby != null) {
	$('.tools > ul > li > button.filter .icon').addClass('on');
	filter_standby = JSON.parse(filter_standby);
	data.param_filter = filter_standby;
}
else{
	$('.tools > ul > li > button.filter .icon').removeClass('on');
}

let img_standby = sessionStorage.getItem(session_img);
if (img_standby != null) {
	if (img_standby == "P") {
		$("ul#list").parent().removeClass("outfit");
		$('main.goods.list > header section.tools button.showing').removeClass('on');
	} else if (img_standby == "O") {
		$("ul#list").parent().addClass("outfit");
		$('main.goods.list > header section.tools button.showing').addClass('on');
	}
}

let grid_standby = sessionStorage.getItem(session_grid);
if (grid_standby != null) {
	if (grid_standby == "2") {
		$("ul#list").parent().addClass("col-2");
		$('main.goods.list > header section.tools button.column').addClass('on');
	} else if (grid_standby == "4") {
...
function setFilter_sz(filter_type,data) {
	...
	if (data != null && data.length > 0) {
		data.forEach(function(row) {
			filter += `<li data-type="SZ" data-no="${row.filter_idx}" data-sort="${row.size_sort}">${row.filter_name}</li>`;
		});
