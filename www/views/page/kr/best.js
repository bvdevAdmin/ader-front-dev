...
const session_order	= `order_best_${data.menu_type}_${data.menu_idx}`
const session_filter	= `filter_best_${data.menu_type}_${data.menu_idx}`
const session_img		= `img_best_${data.menu_type}_${data.menu_idx}`;
const session_grid	= `grid_best_${data.menu_type}_${data.menu_idx}`;

let order_best = sessionStorage.getItem(session_order);
if (order_best != null) {
	order_best = JSON.parse(order_best);
	data.param_sort = order_best;
}

let filter_best = sessionStorage.getItem(session_filter);
if (filter_best != null) {
	$('.tools > ul > li > button.filter .icon').addClass('on');
	filter_best = JSON.parse(filter_best);
	data.param_filter = filter_best;
}
else{
	$('.tools > ul > li > button.filter .icon').removeClass('on');
}

let img_best = sessionStorage.getItem(session_img);
if (img_best != null) {
	if (img_best == "P") {
...
let grid_best = sessionStorage.getItem(session_grid);
if (grid_best != null) {
	if (grid_best == "2") {
			...
			if(d.code == 200) {
				if ($("#goods-nav-top > ul > li").length == 0) {
					...
					$("#goods-nav-top > ul").append(`
						...
						<li>
							<a href="#" class="on">
								<span class="title">실시간 인기 제품</span>
							</a>
						</li>
					`);
				}
				...
				d.data.grid_info.forEach(row => {			
					...
					let product_img_location = (row.product_img) ? config.cdn + row.product_img.product_p_img[0]?.img_location : ''
					$("#list").append(`
