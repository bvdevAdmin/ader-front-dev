		...
		<article id="result-article" class="hidden">
			<div class="result-article-wrap">
				<div class="find-store-btn">
					<img src="/images/svg/store-move.svg" alt="">
					<div class="to-find-store" data-i18n="ss_to_stockist"></div>
				</div>
...
<script>
const store_idx		= location.pathname.split("/")[3];
const store_type	= get_query_string("store_type");
...
document.addEventListener("DOMContentLoaded", function () {
	...
	renderDetailWrap(store_idx, store_type);
});
...
const getStoreList = () => {
	$.ajax({
		...
		error: function () {
			...
			makeMsgNoti(config.language, "MSG_F_ERR_0099", null);
		},
		success: function (d) {
			if(d.code == 200){
				if(!(d.data != null)){
...
function makeSpaceHtml(data) {
	...
	`
		<figure>
			<img class="banner-img" data-src="${contents_info[0].contents_location}" src="${config.cdn}${contents_info[0].contents_location}">
			<figcaption>
				...
				<div class="store_addr-box">
					...
					<a href="${link_map}" target='_blank'><div class="addr-svg" data-link=""><img src="/images/ico-location-o.svg" alt=""><span data-i18n="ss_view_location">View location</span></div></a>
				</div>
				<div class="store_tel">${store_tel}<img src="/images/svg/store-phone.svg" alt=""></div>
				<div class="store_open_date">${store_sale_date}</div>
			</figcaption>
		</figure>
		<div class="detail-view-btn" data-i18n="ss_view_detail">View Details</div>
	`;
		...
		if (backgroundType === "mp4") {
			backgroundHtml += `
				<div class="swiper-slide vplayer">
					<video id="video-coustom-${idx}" autoplay muted loop playsinline src="${config.cdn}${el.contents_location}" "></video>
				</div>
			`;
		} else {
			backgroundHtml +=`
				<div class="swiper-slide">
					<img class="object-fit" src="${config.cdn}${el.contents_location}" ">
				</div>
			`;
		}
	});
	...
	let innerHtml = `
		...
		<figure>
			<figcaption>
				...
				<div class="store_addr-box">
					<div class="store_addr">${store_addr}</div>
					<div class="addr-svg" data-link=""><a class="store_addr_link" href="${link_map}"><img src="/images/ico-location-o.svg" alt=""><span data-i18n="ss_view_location">View location</span></a></div>
				</div>
				<div class="store_tel">${store_tel}<img src="/images/svg/store-phone.svg" alt=""></div>
				<div class="store_open_date">${store_sale_date}</div>
			</figcaption>
