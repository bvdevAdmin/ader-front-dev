	...
	<article>
		<div class="tab">
			...
			<section>
				...
				<button type="button" class="btn black btn_M_add">Regist</button>
			</section>
		</div>
	</article>
</section>
					...
					if (d.code == 200) {
						...
						if ($('input[name="default_flg"]').prop('checked')) {
							...
							delivery_address.forEach(function(row) {
								...
								if (config.language == "EN" && product < 300) {
									delivery_price = parseFloat(row.delivery_price);
								}
...
function caclDelivery_price(delivery_price) {
	...
	if (config.language == "EN") {
		...
		voucher		= parseFloat(discount).toFixed(2);
		mileage		= parseFloat(mileage).toFixed(2);
	}

	let delivery = 0
	if (product < 300) {
		delivery = parseFloat(delivery_price).toFixed(2);
	}
