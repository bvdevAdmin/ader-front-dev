<?php
    ...
    $MOK_RESULT_REDIRECT_URL = "https://stg.adererror.com/kr/find-account";
    ...
    $result = mobileOK_std_result($mobileOK, $MOK_RESULT_REQUEST_URL);

    $member_id = null;

    if (isset($result['user_phone'])) {
        $tel_mobile = null;
        if (preg_match('/^(\d{3})(\d{4})(\d{4})$/',$result['user_phone'],$match)) {
            $tel_mobile = $match[1].'-'.$match[2].'-'.$match[3];
        }

        if ($tel_mobile != null) {
            $member = $db->get("MEMBER","COUNTRY = 'KR' AND TEL_MOBILE = ?",array($tel_mobile));
            if (sizeof($member) > 0) {
    ...
    echo "
        <script>
            sessionStorage.setItem('result_find','".json_encode($result_find, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES)."');
            location.href = '$MOK_RESULT_REDIRECT_URL';
        </script>
    ";
