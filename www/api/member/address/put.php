...
if(!isset($country) || !isset($_SESSION['MEMBER_IDX'])) {
	$code = 401;
	$msg = getMsgToMsgCode($db, $country, 'MSG_B_ERR_0018', array());
}
elseif ($member_idx > 0 && isset($to_zipcode)) {
	...
	try {
		...
		$bool_default_flg = ($default_flg == 'true') ? 1 : 0;	
		...
		$code = 302;
		$msg = getMsgToMsgCode($db, $country, 'MSG_B_ERR_0050', array());		
	}
} else {
	$code = 301;
	$msg = getMsgToMsgCode($db, $country, 'MSG_B_ERR_0032', array());
}
