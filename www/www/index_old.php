...
/*==============================================================
  ...
  ==============================================================*/
foreach(apache_request_headers() as $header => $val) {
    define(strtoupper($header),$val);
    if(strtolower($header) == 'country') {
		$country = $val;
		$_POST['country'] = $val;
		if(!defined('COUNTRY')) define('COUNTRY', $val);
	}
}
...
if(is_array($_CONFIG['M']) && sizeof($_CONFIG['M']) > 1) {
	switch($_CONFIG['M'][0]) {
		case '_js': // 스크립트 요청
			$_CONFIG['M'] = array_splice($_CONFIG['M'],1);
		...
		break;
		case '_modal': // 모달 창
			$inc_path = $_CONFIG['PATH']['MODAL'].$_CONFIG['M'][1].'.php';
			if(!file_exists($inc_path)) {
				$_m = explode('-',$_CONFIG['M'][1]);
				$inc_path = $_CONFIG['PATH']['MODAL'].$_m[0].'-'.$_m[sizeof($_m)-1].'.php';
			}
		break;
		case '_file': // 파일 다운로드
			ob_start();
	...
	if(isset($inc_path) && file_exists($inc_path)) {
		if($_CONFIG['M'][0] == '_api') {
			include $_CONFIG['PATH']['API'].'_legacy/send/common.php';
			include $_CONFIG['PATH']['API'].'_legacy/common.php';
...
if(in_array($_CONFIG['M'][0],array('collaboration')) === true) {
    $_inc_page = $_CONFIG['M'][0];
}
else {
...
if(defined('IS_SCRIPT')) {
	if(file_exists($_CONFIG['PATH']['PAGE'].$_inc_page.'.js')) {
		include $_CONFIG['PATH']['PAGE'].$_inc_page.'.js';
	}
}
elseif(file_exists($_CONFIG['PATH']['PAGE'].$_inc_page.'.php')) {
