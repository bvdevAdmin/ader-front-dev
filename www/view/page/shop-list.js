		...
		success: function(d) {
			if(d.code == 200) {
				...
				d.data.grid_info.forEach(row => {
					...
					// 아이템 <-> 착용샷
					let outfit_image = `style="--outfit-src: url('${config.cdn + row.product_img.product_p_img[0].img_location}'); "`;
					...
					$("#list").append(`
						<li class="${(row.stock_status == 'STSO')?'soldout':''}" style="${(row.background_color!='')?'background-color:' + row.background_color:''}">
							<a href="/shop/${row.product_idx}">
								<span 
									class="image" 
									data-src="${config.cdn + row.product_img.product_p_img[0].img_location}"
									${outfit_image}
