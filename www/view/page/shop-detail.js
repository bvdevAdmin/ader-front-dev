...
$(document).ready(function() {
	let goods_no = location.pathname.split("/")[2]; // 상품 인덱스키
							...
							$("#option-color ul").append(`
								<li class="${(row.stock_status == 'STSO')?'soldout':''} ${('/shop/' + row.product_idx == location.pathname)?'now':''}">
									<label>
										...
										<a href="/shop/${row.product_idx}" class="colorchip"><span class="${row.color.toLowerCase()}" style="background-color:${row.color_rgb}"></span></a>
									</label>
								</li>
							`);
						});
